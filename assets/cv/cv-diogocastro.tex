...
% Used to ensure two `cventry`s appear together on the same page.
...
% (it's numbering pages as x/4 when there are only 3 pages in total).
%
...
\cvitem{}{
  ...
  More recently, I've also been using Rust for commercial projects.
  \para
...
\cventry{April 2020-Present}{Team Lead, Software Engineer}{Serokell}{Remote}{}{
  Led the internal development team in developing several open-source Haskell libraries, such as
  ...
  a platform for assessing a candidate's Haskell skills (now operated by the Haskell Foundation).
  \para
  Developed a Rust application using Tauri for controlling
  Molecular Rotational Resonance (MRR) spectroscopy instruments,
  performing measurements, and visualizing and analyzing the measurement data.
  \para
  Helped grow the Haskell and Rust ecosystems for the Tezos blockchain by developing
  a parser, typechecker and interpreter for the Michelson language, a REPL,
  and a test framework for smart contracts.%
  \para
  Helped improve and diagnose performance issues in a PostgreSQL / AlloyDB event-sourced database.
  \para
  Setup instrumentation for Rust applications using VictoriaMetrics and Grafana.
  \para
...
% Keep these two `cventry`s together on the same page.
% If not enough vertical space can be reserved, then move both to the next page.
...
\cventry{}{Senior Software Engineer}{SpotX}{Belfast, UK}{}{
  ...
  with the goal of preparing our engineers to be productive in Scala while
  building an intuition of how to program with functions and equational
  reasoning.
  \para
  Trained new hires and helped them bridge the gap between what they already knew
...
\cventry{}{Software Engineer}{NantHealth}{Belfast, UK}{}{
  ...
  people talked about topics that interested them.
  I sometimes brought an exercise for people to
...
\cventry{Aug 2012-Mar 2013}{Researcher}{Fraunhofer}{Porto, Portugal}{}{
  ...
  Developed complex heuristics to evaluate landmark data retrieved from OpenStreetMap,
...
\section{Open Source}
\project{linear-locks}{https://hackage.haskell.org/package/linear-locks}{
...
\project{haskell-flatbuffers}{https://hackage.haskell.org/package/flatbuffers}{
  ...
  serialization, originally designed by Google.
...
\project{lsp-xreferee}{https://github.com/dcastro/lsp-xreferee}{
  LSP server written in Haskell for validating cross-references throughout a git repository.
}
\project{tztime}{https://hackage.haskell.org/package/tztime}{
  Haskell library for safely manipulating dates and time in a timezone-aware fashion, avoiding common pitfalls
  such as ambiguous or invalid datetimes.
}
\project{tzbot}{https://github.com/serokell/tzbot}{
  Slack bot that detects messages with references to some point in time and automatically converts
  them to the reader's timezone.
}
